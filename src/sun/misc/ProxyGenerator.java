    ...
    */
    ...
    private byte[] generateClassFile() {
        ...
        try {
            ...
            //2.写入次版本号                            // u2 minor_version;
            dout.writeShort(CLASSFILE_MINOR_VERSION);
            //3.写入主版本号                            // u2 major_version;
            dout.writeShort(CLASSFILE_MAJOR_VERSION);
            //4.写入常量池
            cp.write(dout);             // (write constant pool)

            //5.写入访问修饰符                          // u2 access_flags;
            dout.writeShort(accessFlags);
            //6.写入类索引                          // u2 this_class;
            dout.writeShort(cp.getClass(dotToSlash(className)));
            //7.写入父类索引, 生成的代理类都继承自Proxy                          // u2 super_class;
            dout.writeShort(cp.getClass(superclassName));

            //8.写入接口计数值                          // u2 interfaces_count;
            dout.writeShort(interfaces.length);
            //9.写入接口集合                           // u2 interfaces[interfaces_count];
            for (Class<?> intf : interfaces) {
                dout.writeShort(cp.getClass(
                    dotToSlash(intf.getName())));
            }

            //10.写入字段计数值                           // u2 fields_count;
            dout.writeShort(fields.size());
            //11.写入字段集合                            // field_info fields[fields_count];
            for (FieldInfo f : fields) {
                f.write(dout);
            }

            //12.写入方法计数值                          // u2 methods_count;
            dout.writeShort(methods.size());
            //13.写入方法集合                          // method_info methods[methods_count];
            for (MethodInfo m : methods) {
                m.write(dout);
            }

            //14.写入属性计数值, 代理类class文件没有属性所以为0                           // u2 attributes_count;
